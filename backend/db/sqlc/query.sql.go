...
const getClicksByUserGroupedByMonth = `-- name: GetClicksByUserGroupedByMonth :many
SELECT
...
type GetClicksByUserGroupedByMonthRow struct {
	Month      pgtype.Timestamp
	ClickCount int64
}

func (q *Queries) GetClicksByUserGroupedByMonth(ctx context.Context, userID int64) ([]GetClicksByUserGroupedByMonthRow, error) {
	rows, err := q.db.Query(ctx, getClicksByUserGroupedByMonth, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetClicksByUserGroupedByMonthRow
	for rows.Next() {
		var i GetClicksByUserGroupedByMonthRow
...
const getMostClickedUrls = `-- name: GetMostClickedUrls :many
SELECT 
    u.id,
    u.long_url,
    u.code,
...
type GetMostClickedUrlsRow struct {
	ID         int64
	LongUrl    string
	Code       string
	ClickCount int64
...
func (q *Queries) GetMostClickedUrls(ctx context.Context, userID int64) ([]GetMostClickedUrlsRow, error) {
	rows, err := q.db.Query(ctx, getMostClickedUrls, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetMostClickedUrlsRow
	for rows.Next() {
		var i GetMostClickedUrlsRow
		if err := rows.Scan(
			&i.ID,
			&i.LongUrl,
			&i.Code,
